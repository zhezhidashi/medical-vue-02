<template>
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <el-dialog title="修改机构信息" :visible.sync="modifyInstitutionDialogVisible" width="50%">
            <el-form :model="institutionInfo" label-width="auto" >
                <el-form-item prop="name" label="机构名称">
                    <el-input v-model="institutionInfo.name"></el-input>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button @click="modifyInstitutionCancel">取 消</el-button>
                <el-button type="primary" @click="modifyInstitutionConfirm">确 定</el-button>
            </span>
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            institutionName: '',
            modifyInstitutionDialogVisible: false,
        }  
    },
    mounted() {
    ...
    methods: {
        ...
        modifyInstitution() {
            this.modifyInstitutionDialogVisible = true;
        },
        modifyInstitutionCancel() {
            this.$confirm('不保存而直接关闭可能会丢失本次编辑的信息，是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                this.modifyInstitutionDialogVisible = false;
                this.institutionInfo.name = '';
            }).catch(() => {
                this.$message({
                    type: 'info',
                    message: '已取消'
        ...
        modifyInstitutionConfirm() {
            if (this.institutionInfo.name === '') {
                this.$message.error('请填写完整信息');
                return;
            }
            ...
            postForm('/networkGroups/setInstitutionName', postData, _this, function(res){
                if (res.code === 200) {
                    _this.$message.success('修改成功');
                    _this.modifyInstitutionDialogVisible = false;
                    _this.getInstitutionName();
                }
                else {
                    _this.$message.error(res.message);
                }
